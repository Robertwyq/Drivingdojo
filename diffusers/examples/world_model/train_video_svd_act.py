...
# ours code base
from utils.video_datasets import VideoCondDataset
...
def log_validation(vae, condition_encoder, unet, args, accelerator, weight_dtype, epoch):
    ...
    pipeline = StableVideoDiffusionPipelineV2.from_pretrained(
...
def main():
    ...
    with ContextManagers(deepspeed_zero_init_disabled_context_manager()):
        ...
        layout_encoder = EgoCondEncoder(out_dim=1024)
    # Freeze 
    vae.requires_grad_(False)
    image_encoder.requires_grad_(False)
    layout_encoder.requires_grad_(True)
        ...
        def load_model_hook(models, input_dir):
            ...
            for i in range(len(models)):
                ...
                if i==2:
                    ...
                    load_model = EgoCondEncoder.from_pretrained(input_dir, subfolder="layout_encoder")
    ...
    # Initialize the optimizer
    all_parameters = list(unet.parameters())+list(filter(lambda p: p.requires_grad, layout_encoder.parameters()))
    ...
    if args.dataset_name is not None:
        if args.load_from_pkl:
            ...
            train_dataset = VideoCondDataset(
    ...
    unet, optimizer, train_dataloader, lr_scheduler, image_encoder, layout_encoder= accelerator.prepare(
        unet, optimizer, train_dataloader, lr_scheduler, image_encoder, layout_encoder
    )
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                box_info, map_info, bev_info, text_info, ego_info = None, None, None, None, None
                # processing condition
                if 'box' in args.conditions:
                    box_info = batch['box_values'].reshape(-1,*batch['box_values'].shape[-3:])
                if 'map' in args.conditions:
                    map_info = batch['map_values'].reshape(-1,*batch['map_values'].shape[-3:])
                if 'bev' in args.conditions:
                    bev_info = batch['bev_values'].reshape(-1,*batch['bev_values'].shape[-3:])
                if 'text' in args.conditions:
                    text_info = batch['text_values']
                if 'ego' in args.conditions:
                    ego_info = batch['ego_values'].reshape(-1,*batch['ego_values'].shape[-1:])
                
                layout_latents = layout_encoder(ego = ego_info)
                ...
                # condition
                image_embeddings_temporal = image_embeddings.repeat_interleave(args.nframes, dim=0)
    ...
    if accelerator.is_main_process:
        ...
        if layout_encoder is not None:
            layout_encoder = accelerator.unwrap_model(layout_encoder)

        pipeline = StableVideoDiffusionPipelineV2(
